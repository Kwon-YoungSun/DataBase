...
/*
    ...
    우리가 사용하는 프로그램에서 주석처리가 가능한 것은
    단지 이 프로그램에서만 주석으로 처리한다는 것이다.
    
    참고 ]
        sql은 rdbms에서만 사용 가능한 언어이다.
        nosql
    ------------------------------------------------------------------------
    ...
    원래 sql 문서에는 여러 줄 주석이 없다.
    그러나 오라클에는 자바 라이브러리가 포함되어 있기 때문에
    자바 구문이 사용 가능하다.
    
    ------------------------------------------------------------------------
    
        우리가 설치할 때 SID를 정하는 부분이 있었다.
        
        오라클은 데이터베이스 내부에
        사용자가 만들어지고
        사용자 내부에 다시 데이터를 저장할 수 있는 테이블들이 만들어진다.
    
        이때 데이터베이스를 한개만 만들 수 있는 것은 아니다.
...
/*
    ...
    용어 정리 ]
        ...
        DBMS        - 데이터베이스 관리 시스템
        Data Bank   - 데이터를 보관하는 장소를 관리하는 도구
        
        참고 ]
            흔히 좋은 데이터베이스, 성능이 좋다 나쁘다
            라는 표현은
            DBMS의 기능에 따라서 구분되어지는 것이다.
            따라서 데이터베이스 프로그램마다 DBMS가 다르다는 말이고
...
RDBMS (관계형 데이터베이스 시스템) 에서 데이터를 보관하는 방법
    ...
    --> 테이블이란
        ...
        필드와 레코드로 구성된 데이터를 보관하는 작은 단위
            ...
            필드 - 같은 속성의 데이터 모임(칸) - COLUMN <-- ATTRIBUTE
...
/*  
    접속한 계정내의 테이블을 확인하는 방법
        형식 ]
            
            SELECT tname FROM tab;
    참고 ]
        오라클은 명령을 내릴 때 대소문자를 전혀 구분하지 않는다.
        
        다만
        가독성 등의 이유 때문에
        기본 명령은 대문자로 하고 필드이름, 테이블이름은 소문자로
        나머지는 대문자로 사용할 예정이다.
*/
...
/*
    ...
    참고 ]
        sqlplus에서만 사용할 수 있는 sqlplus명령들은 세미콜론을 붙이지 않아도 된다.
...
/*
    같은 데이터가 중복되어서 나오는 것을 방지하고자 할 때 사용하는 명령
...
SELECT
    empno, ename, sal, sal + 100, 2 * 4 -- 연산식의 결과는 데이터의 갯수만큼 출력된다.
FROM
    emp
;
...
-- 모든 사원들의 사원이름, 커미션, 커미션에 500을 추가한 커미션을 조회하세요.
...
/*
    세션이 정상적으로 종료되었을 때에만 자동 커밋을 해준다.
    비정상적으로 종료되었을 때 세션이 닫히지 않고 계속 열려 있는 상태로 남아 있게 된다.
    따라서 다시 세션에 접속했을 때, 생성되는 새로운 세션은 비정상적으로 종료된 세션과
    다른 별도의 세션이다.
*/
...
/*
    필드이름 변경하기
        ...
        그런데 위의 예제처럼 필드의 이름이 복잡해지는 경우도 발생한다.
...
SELECT
    empno "사원 번호", (ename || ' 사원') "사원 이름", NVL(job, '신입사원') "사원 직급"
FROM
    emp
;
...
/*
    emp 테이블
    ==> 이 테이블은 오라클이 학습용으로 제공하는 샘플 테이블이다.
...
WHERE
    deptno = 10
;
-- 팁 ] WHERE 절 조건은, 데이터를 많이 거를 수 있는 순서대로 집어넣자.

SELECT
    사원번호, 사원이름, 상사이름
FROM
    (SELECT
        e.empno 사원번호, (e.ename || ' 사원') 사원이름, e.mgr 상사번호, 
        ee.empno 상사사원번호, ee.ename 상사이름
    FROM
        emp e, emp ee
    WHERE
        e.mgr = ee.empno) -- 조인과 같음
;
...
/*
    ...
    원칙적으로 문자는 문자끼리만 비교할 수 있고 숫자는 숫자끼리만 비교할 수 있다.
    단, 예외가 있는데
...
-- 급여가 1500 이상인 사원의 이름, 직책, 급여를 조회하세요. 별칭으로 표현되게 하세요.
SELECT
...
-- 사원이름이 M 이후의 문자로 시작하는 사원의 사원번호, 사원이름, 사원직급을 조회하세요.
-- 단, 별칭으로 출력되게 하세요.
SELECT
...
WHERE
    ename >= 'N'
--  ename > 'Ma' -- 'Mettue'일 경우 출력됨에 주의
--  ename > 'MZZZZZZZZZZZZZZ'
    -- 주의! 'M' < 'MA' 임을 주의하자.
;
...
--  NOT deptno = 10
--  deptno <> 10 -- 정식 문법
    deptno != 10 -- oracle에서만 허용됨(SQL 문에는 없음)
;
