...
-------------------------------------------------------------------------------------------------------------------------------------
    ...
    인덱스 만들기
        ...
        형식 2 ] --> UNIQUE 인덱스 만들기
            ==> 인덱스용 데이터가 반드시 UNIQUE 핟다는 보장이 있을 때 만드는 방법
        ...
        형식 4 ] --> 비트 인덱스
            ==> 주로 필드의 데이터의 갯수가 정해진 경우에 사용하는 방법
                    ...
                    예 ]
                        EMP 테이블의 DEPTNO의 경우는 10, 20, 30, 40 만 입력할 수 있다.
...
-- EMP 테이블을 복사해서 EMP00 이라는 테이블을 만들어보자.
CREATE TABLE emp00
AS
    SELECT
        *
...
ALTER TABLE emp00
ADD CONSTRAINT EMP_NO_PK2 PRIMARY KEY(empno);

ALTER TABLE
    emp00
ADD CONSTRAINT EMP_DNO_FK2 FOREIGN KEY(deptno) REFERENCES dept(deptno);

-- 시퀀스를 만든다. 시작값은 8000, 증가값은 1씩 증가하고 최댓값은 9999, 반복과 캐쉬기능은 허용하지 않는다.
...
/*
    sqlplus 접속 명령
        ...
        명령 1 ] --> 로컬에 오라클이 설치되어있는 경우
            sqlplus 계정이름/비번

        방법 2 ] --> 다른 컴퓨터에 오라클이 설치되어있는 경우
            sqlplus 계정이름/비번@DB서버주소:포트번호/SID
            
        sqlplus에서 실행 시간을 표시하는 방법 : set timing on
*/
...
FROM
    emp00
; -- 38.215초 <== 인덱스를 수정하는 작업이 추가되서 느려졌다.

CREATE BITMAP INDEX dno_idx01
ON
    emp00(deptno);
    
SELECT DISTINCT deptno 부서번호 FROM emp00; -- 0.181초

SELECT DISTINCT deptno 부서번호 FROM emp01; -- 0.116초 
...
---------------------------------------------------------------------------------------------------------------------------------                                                                    
                ...
                1. 사용자 계정 만들기
                    ...
                    1. 관리자 계정으로 접속한다.
                        ...
                        1) sqlplus 의 경우
                            ...
                            b)  sqlplus / as sysdba
                                ...
                                sqlplus hello@localhost:1521/orcl
                                
                                conn system/increpas@localhost:1521/orcl as sysdba
                    ...
                    2. 사용자 만든다.
                            ...
                            참고 ]
                                계정은 만들어 두면
                                만든 계정은 아무 권한도 가지고 있지 않은 상태가 된다.
                ...
                2. 권한 부여하기
                    ...
                    참고 ]
                        ...
                        계정을 만든 직후에 따로 세션을 만들 수 있는 권한을 부여받지 않으면
                        오라클에 접속할 수 없는 상태이다.
...
--------------------------------------------------------------------------------------------------------------------------
    ...
    참고 ]
        권한을 부여할 때 사용되는 옵션
            ...
            2. WITH GRANT OPTION
                ==> 관리자에게 부여받은 권한을 다른 계정에게 전파할 수 있는 권한이다.
...
--------------------------------------------------------------------------------------------------------------------------------------------
            ...
            계정 삭제하기
                ...
                형식 ]
                    DROP USER 계정이름 CASCADE; <-- CASCADE는 계정 안의 테이블까지 모두 삭제하는 명령임
...
-------------------------------------------------------------------------------------------------------------------------------------------
            ...
            롤을 이용한 권한 부여 방법
                1. 이미 만들어진 롤을 이용하는 방법
                    ...
                    1) CONNECT
                    ==> 주로 CREATE와 관련된 권한을 모아놓은 ROLE 이다.
                            세션 접속에 관련된 권한들의 묶음
...
--> SYSTEM 계정으로 실행한다.
drop synonym deptsal;

-- 2.
CREATE PUBLIC SYNONYM deptsal
FOR hello.dsal;
