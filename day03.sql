...
SELECT
    TO_CHAR(456789, '000,000,000.00') NO1, TO_CHAR(456789, '999,999,999.99') NO2
FROM
    dual
;
...
-- 왼쪽에 #을 붙인다.(LPAD)
    CONCAT( 
        ...
        RPAD(
            LPAD(
                SUBSTR(TO_CHAR(sal, '000,000'), -5, 2),
                4, '#'),
            7, '#')
    ) 급여
FROM
...
SELECT
    empno 사원번호,
    RPAD( -- RPAD로 3번째 문자의 오른쪽 나머지 문자들을 *로 채우기
        LPAD(
            SUBSTR(ename, 3, 1), 3, '*'), -- 앞에서부터 두글자를 LPAD로 *를 채우기
        LENGTH(ename), '*'
    ) 이름
FROM
    emp
...
SELECT
    TO_CHAR(sysdate, 'yyyy/MM/dd DAY HH:mi:ss') 오늘날짜
FROM
    dual
;
...
SELECT
    FLOOR(sysdate - TO_DATE('2020/07/15', 'YYYY/MM/DD')) || ' 일' 수업일수
FROM
    dual
;
...
*/
-- 자신이 태어난 날 부터 몇개월이 되었는지 조회하세요.
SELECT
    FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE('1999/05/24', 'YYYY/MM/DD'))) 개월수
FROM
    dual
;
...
SELECT
    ename 사원이름,
    hiredate 입사일,
    FLOOR(MONTHS_BETWEEN(SYSDATE, hiredate)) || '개월' "몇개월 전?"
FROM
    emp
;
...
-- 올 2월의 마지막 날짜를 조회하세요.
SELECT
    LAST_DAY(TO_DATE('2020/02','YYYY/MM')) "2월 마지막 날"
FROM
    dual
;
...
SELECT
    TO_CHAR(ROUND(SYSDATE, 'MI'), 'YYYY/MM/DD HH24:MI:SS') 시간반올림
FROM
    dual
;
...
SELECT
    CONCAT(
        CONCAT('사원이름 : ', LPAD(ename, 7, ' ')) || ', ' , -- 사원이름
        CONCAT('사원급여 : ', -- 사원급여
            CONCAT(
                LPAD(
                    SUBSTR(TO_CHAR(sal, '000,000'), -4, 1),
                    4, '*'), -- ',' 왼쪽에는 *만 채운다.
                LPAD(
                    SUBSTR(TO_CHAR(sal, '000,000'), -2),
                    3, '*') -- ',' 오른쪽에는 뒤의 두 자리만 남겨놓고 나머지를 *로 채운다.
            )
        ) 
    ) 사원정보
FROM
...
FROM
    emp
WHERE
    TO_CHAR(hiredate) LIKE '__/05/__'
;
...
SELECT
    ename 이름, CONCAT('$', TO_CHAR(sal, '9,999')) 급여
FROM
    emp
;
...
SELECT
    ename, hiredate 입사일, TO_CHAR(hiredate, 'DAY') 입사요일
FROM
    emp
;
...
SELECT FLOOR(sysdate - TO_DATE('1999/05/24')) "살아온일수" FROM dual;

SELECT FLOOR(sysdate - TO_DATE('05/24/1999', 'mm/dd/yyyy')) "살아온일수" FROM dual;

/*
    3. TO_NUMBER
        ==> 문자로 된 내용을 숫자 데이터로 변환시켜주는 함수
            <== 문자데이터는 +, - 연산이 되지 않는다.
...
SELECT
    TO_NUMBER('123,546', '999,999') - TO_NUMBER('5,678', '9,999') RESULT
FROM
    dual
;
    ...
    기타함수
        ...
        4. COALESCE
            ...
            의미 ]
                ...
                (만약 데이터1이 NULL이라면, 데이터2를 출력한다.
                데이터2가 NULL이라면, 데이터3을 출력한다., ...)
                
*/

SELECT
    ename, comm, NVL(TO_CHAR(COMM + 100) , '커미션없음') 변경커미션
FROM
    emp
;
...
SELECT
    ...
    comm 커미션,
    sal 급여,
--    NVL2(comm, sal*1.1 + comm, sal*1.05 + 100) 인상금액
--    NVL(comm + sal*1.1 , sal*1.05+100) 인상금액
      COALESCE(comm + sal * 1.1, sal*1.05+100) 인상금액
FROM
    emp
;
/*
...
SELECT
    ...
    comm 커미션,
    sal 급여,
--    NVL2(comm, comm * 1.5, sal * 0.1) 추가지급
--    NVL(comm * 1.5, sal*0.1) 추가지급
      COALESCE(comm * 1.5, sal * 0.1) 추가지급
FROM
    emp
;
